...
2. **`code-reviewer`** — reviews the changes for correctness and standards compliance
3. **`test-runner`** — builds the package and runs tests to verify nothing is broken

All three steps are mandatory. Do not skip the reviewer or test-runner, even for "simple" changes. Do not present coder output to the user without review and build verification.
