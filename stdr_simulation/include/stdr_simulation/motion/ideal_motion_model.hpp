...
public:
   ...
   */
  explicit IdealMotionModel(std::uint32_t seed);
  ...
  /**
   * @brief Noise-free exact arc integration of a command over one timestep.
   *
...
private:
  // Shared exact-arc kinematics used by both update() (noisy) and integrate()
  // (noise-free) so the geometry lives in exactly one place.
  [[nodiscard]] static Pose2D integrate_kinematics(const Pose2D& current, double v, double w, double dt,
