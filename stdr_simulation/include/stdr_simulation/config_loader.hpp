 ...
 * @return The assembled RobotConfig, or nullopt if parsing fails.
 */
[[nodiscard]] std::optional<RobotConfig> load_robot_config(
 ...
 * @return MapMetadata with a resolved image path, or nullopt if parsing fails.
 */
[[nodiscard]] std::optional<MapMetadata> load_map_metadata(
    const std::string& yaml_path);
