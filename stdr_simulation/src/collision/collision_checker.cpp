...
std::vector<std::pair<int, int>> CollisionChecker::get_points_between(
    int x1, int y1, int x2, int y2) {
  std::vector<std::pair<int, int>> points;

  const double angle = std::atan2(static_cast<double>(y2 - y1), static_cast<double>(x2 - x1));
  const double dist =
      std::hypot(static_cast<double>(x2 - x1), static_cast<double>(y2 - y1));
...
bool CollisionChecker::check_collision(
  ...
  for (const Point2D& pt : fp_points) {
    ...
    const std::size_t idx =
        static_cast<std::size_t>(grid_y) * static_cast<std::size_t>(map.width) +
...
bool CollisionChecker::check_path_collision(
  ...
  const double path_angle =
      std::atan2(static_cast<double>(new_gy - prev_gy), static_cast<double>(new_gx - prev_gx));
  const double path_dist =
      std::hypot(static_cast<double>(new_gx - prev_gx), static_cast<double>(new_gy - prev_gy));
  ...
  while (d <= path_dist) {
    ...
    for (std::size_t i = 0; i < n; ++i) {
      ...
      const int ax =
          step_x + static_cast<int>((pa.x * cos_theta - pa.y * sin_theta) / map.resolution);
      const int ay =
          step_y + static_cast<int>((pa.x * sin_theta + pa.y * cos_theta) / map.resolution);
      const int bx =
          step_x + static_cast<int>((pb.x * cos_theta - pb.y * sin_theta) / map.resolution);
      const int by =
          step_y + static_cast<int>((pb.x * sin_theta + pb.y * cos_theta) / map.resolution);
      ...
      for (const auto& [cx, cy] : edge_cells) {
        ...
        for (int dy = -1; dy <= 1; ++dy) {
          for (int dx = -1; dx <= 1; ++dx) {
            ...
            const std::size_t idx =
                static_cast<std::size_t>(ny) * static_cast<std::size_t>(map.width) +
