...
LaserScan LaserSimulator::simulate(const Pose2D& sensor_pose_world, const LaserConfig& config,
                                   const OccupancyGrid& map) const
{
  ...
  const double angle_increment =
      (config.num_rays > 1) ? (config.max_angle - config.min_angle) / static_cast<double>(config.num_rays - 1) : 0.0;
  ...
  // Treating it as a bias shifts every beam by a constant offset, which
  // introduces systematic range error rather than realistic sensor noise.
  ...
  for (std::int32_t ray_idx = 0; ray_idx < config.num_rays; ++ray_idx)
  {
    const double ray_angle =
        sensor_pose_world.theta + config.min_angle + static_cast<double>(ray_idx) * angle_increment;
