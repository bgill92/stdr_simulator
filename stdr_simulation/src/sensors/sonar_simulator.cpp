...
SonarScan SonarSimulator::simulate(const Pose2D& sensor_pose_world,
  ...
  for (double angle = -config.cone_angle / 2.0;
       angle <= config.cone_angle / 2.0;
       angle += kAngleStepRad) {
    ...
    for (int step = 1; step <= max_steps; ++step) {
      const int cell_x = static_cast<int>(
          origin_grid_x + cos_angle * static_cast<double>(step));
      const int cell_y = static_cast<int>(
          origin_grid_y + sin_angle * static_cast<double>(step));
