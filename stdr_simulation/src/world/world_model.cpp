...
void WorldModel::remove_rfid_tag(const std::string& id) {
  std::erase_if(rfid_tags_, [&id](const stdr_simulation::RfidTag& t) {
    return t.tag_id == id;
  });
}
...
void WorldModel::remove_co2_source(const std::string& id) {
  std::erase_if(co2_sources_, [&id](const stdr_simulation::CO2Source& s) {
    return s.id == id;
  });
}
...
void WorldModel::remove_thermal_source(const std::string& id) {
  std::erase_if(thermal_sources_, [&id](const stdr_simulation::ThermalSource& s) {
    return s.id == id;
  });
}
...
void WorldModel::remove_sound_source(const std::string& id) {
  std::erase_if(sound_sources_, [&id](const stdr_simulation::SoundSource& s) {
    return s.id == id;
  });
}
