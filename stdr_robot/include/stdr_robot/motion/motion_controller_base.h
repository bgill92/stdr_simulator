...
public:
  ...
  **/
  virtual void sampleVelocities(void)
  {
    ...
    float sample_ux =
        _motion_parameters.a_ux_ux * ux * ux + _motion_parameters.a_ux_uy * uy * uy + _motion_parameters.a_ux_w * w * w;
    _currentTwist.linear.x += sampleNormal(sqrt(sample_ux));

    float sample_uy =
        _motion_parameters.a_uy_ux * ux * ux + _motion_parameters.a_uy_uy * uy * uy + _motion_parameters.a_uy_w * w * w;
    _currentTwist.linear.y += sampleNormal(sqrt(sample_uy));

    float sample_w =
        _motion_parameters.a_w_ux * ux * ux + _motion_parameters.a_w_uy * uy * uy + _motion_parameters.a_w_w * w * w;
    _currentTwist.angular.z += sampleNormal(sqrt(sample_w));

    float sample_g =
        _motion_parameters.a_g_ux * ux * ux + _motion_parameters.a_g_uy * uy * uy + _motion_parameters.a_g_w * w * w;
...
protected:
  ...
  **/
  MotionController(const geometry_msgs::Pose2D& pose, tf::TransformBroadcaster& tf, const std::string& name,
                   ros::NodeHandle& n, const stdr_msgs::KinematicMsg params)
    : _tfBroadcaster(tf), _freq(0.1), _namespace(name), _pose(pose), _motion_parameters(params)
  {
    _velocitySubscrider = n.subscribe(_namespace + "/cmd_vel", 1, &MotionController::velocityCallback, this);
