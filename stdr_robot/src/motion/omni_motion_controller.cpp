...
@return void
**/
OmniMotionController::OmniMotionController(const geometry_msgs::Pose2D& pose, tf::TransformBroadcaster& tf,
                                           ros::NodeHandle& n, const std::string& name,
                                           const stdr_msgs::KinematicMsg params)
  : MotionController(pose, tf, name, n, params)
{
  _calcTimer = n.createTimer(_freq, &OmniMotionController::calculateMotion, this);
}
...
void OmniMotionController::calculateMotion(const ros::TimerEvent& event)
{
  ...
  if (_currentTwist.angular.z != 0 || _currentTwist.linear.x != 0 || _currentTwist.linear.y != 0)
  {
    ...
    _pose.y += _currentTwist.linear.y * dt.toSec() * sinf(_pose.theta + M_PI / 2.0) +
