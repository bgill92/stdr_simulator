...
@return void
**/
IdealMotionController::IdealMotionController(const geometry_msgs::Pose2D& pose, tf::TransformBroadcaster& tf,
                                             ros::NodeHandle& n, const std::string& name,
...
void IdealMotionController::calculateMotion(const ros::TimerEvent& event)
{
  ...
  {
    _pose.x +=
        -_currentTwist.linear.x / _currentTwist.angular.z * sinf(_pose.theta) +
        _currentTwist.linear.x / _currentTwist.angular.z * sinf(_pose.theta + dt.toSec() * _currentTwist.angular.z);

    _pose.y -=
        -_currentTwist.linear.x / _currentTwist.angular.z * cosf(_pose.theta) +
        _currentTwist.linear.x / _currentTwist.angular.z * cosf(_pose.theta + dt.toSec() * _currentTwist.angular.z);
  }
