...
void Robot::onInit()
{
  ...
  _tfTimer = n.createTimer(ros::Duration(0.1), &Robot::publishTransforms, this, false, false);
}
                            ...
                            const stdr_msgs::RegisterRobotResultConstPtr result)
{
  ...
  for (unsigned int rfidReaderIter = 0; rfidReaderIter < result->description.rfidSensors.size(); rfidReaderIter++)
  {
    _sensors.push_back(SensorPtr(new RfidReader(_map, result->description.rfidSensors[rfidReaderIter], getName(), n)));
  }
  for (unsigned int co2SensorIter = 0; co2SensorIter < result->description.co2Sensors.size(); co2SensorIter++)
  {
    _sensors.push_back(SensorPtr(new CO2Sensor(_map, result->description.co2Sensors[co2SensorIter], getName(), n)));
  }
  for (unsigned int thermalSensorIter = 0; thermalSensorIter < result->description.thermalSensors.size();
       thermalSensorIter++)
  {
    _sensors.push_back(
        SensorPtr(new ThermalSensor(_map, result->description.thermalSensors[thermalSensorIter], getName(), n)));
  }
  for (unsigned int soundSensorIter = 0; soundSensorIter < result->description.soundSensors.size(); soundSensorIter++)
  {
    _sensors.push_back(
        SensorPtr(new SoundSensor(_map, result->description.soundSensors[soundSensorIter], getName(), n)));
  }
  ...
  {
    for (unsigned int i = 0; i < result->description.footprint.points.size(); i++)
    {
  ...
  {
    _motionControllerPtr.reset(new IdealMotionController(_currentPose, _tfBroadcaster, n, getName(), p));
  }
  else if (motion_model == "omni")
  {
    _motionControllerPtr.reset(new OmniMotionController(_currentPose, _tfBroadcaster, n, getName(), p));
  }
  else
  {
...
@return bool
**/
bool Robot::moveRobotCallback(stdr_msgs::MoveRobot::Request& req, stdr_msgs::MoveRobot::Response& res)
{
  if (collisionExistsNoPath(req.newPose) || checkUnknownOccupancy(req.newPose))
  {
    return false;
  }
...
bool Robot::collisionExistsNoPath(const geometry_msgs::Pose2D& newPose)
{
  ...
  {
    double x = _footprint[i].first * cos(newPose.theta) - _footprint[i].second * sin(newPose.theta);
    double y = _footprint[i].first * sin(newPose.theta) + _footprint[i].second * cos(newPose.theta);
...
**/
std::vector<std::pair<int, int> > Robot::getPointsBetween(int x1, int y1, int x2, int y2)
{
...
@return True on collision
**/
bool Robot::collisionExists(const geometry_msgs::Pose2D& newPose, const geometry_msgs::Pose2D& previousPose)
{
  if (_map.info.width == 0 || _map.info.height == 0)
    return false;
  ...
  xMapPrev = movingForward ? (int)(previousPose.x / _map.info.resolution) : ceil(previousPose.x / _map.info.resolution);
  xMap = movingForward ? ceil(newPose.x / _map.info.resolution) : (int)(newPose.x / _map.info.resolution);

  yMapPrev = movingUpward ? (int)(previousPose.y / _map.info.resolution) : ceil(previousPose.y / _map.info.resolution);
  yMap = movingUpward ? ceil(newPose.y / _map.info.resolution) : (int)(newPose.y / _map.info.resolution);
  ...
  {
    x = xMapPrev + (movingForward ? ceil(cos(angle) * d) : (int)(cos(angle) * d));
    y = yMapPrev + (movingUpward ? ceil(sin(angle) * d) : (int)(sin(angle) * d));
    ...
    for (unsigned int i = 0; i < _footprint.size(); i++)
    {
      ...
      double footprint_x_1 =
          _footprint[index_1].first * cos(newPose.theta) - _footprint[index_1].second * sin(newPose.theta);
      double footprint_y_1 =
          _footprint[index_1].first * sin(newPose.theta) + _footprint[index_1].second * cos(newPose.theta);

      int xx1 = x + footprint_x_1 / _map.info.resolution;
      int yy1 = y + footprint_y_1 / _map.info.resolution;

      double footprint_x_2 =
          _footprint[index_2].first * cos(newPose.theta) - _footprint[index_2].second * sin(newPose.theta);
      double footprint_y_2 =
          _footprint[index_2].first * sin(newPose.theta) + _footprint[index_2].second * cos(newPose.theta);
      ...
      std::vector<std::pair<int, int> > pts = getPointsBetween(xx1, yy1, xx2, yy2);

      for (unsigned int j = 0; j < pts.size(); j++)
      {
        static int OF = 1;
        if (_map.data[(pts[j].second - OF) * _map.info.width + pts[j].first - OF] > 70 ||
            _map.data[(pts[j].second - OF) * _map.info.width + pts[j].first] > 70 ||
            _map.data[(pts[j].second - OF) * _map.info.width + pts[j].first + OF] > 70 ||
            _map.data[(pts[j].second) * _map.info.width + pts[j].first - OF] > 70 ||
            _map.data[(pts[j].second) * _map.info.width + pts[j].first + OF] > 70 ||
            _map.data[(pts[j].second + OF) * _map.info.width + pts[j].first - OF] > 70 ||
            _map.data[(pts[j].second + OF) * _map.info.width + pts[j].first] > 70 ||
            _map.data[(pts[j].second + OF) * _map.info.width + pts[j].first + OF] > 70)
    ...
    if ((movingForward && xMap < x) || (movingUpward && yMap < y) || (!movingForward && xMap > x) ||
        (!movingUpward && yMap > y))
    {
      break;
    }
...
void Robot::publishTransforms(const ros::TimerEvent&)
{
  ...
  odom.pose.pose.orientation = tf::createQuaternionMsgFromYaw(_previousPose.theta);
  ...
  for (int i = 0; i < _sensors.size(); i++)
  {
    ...
    _tfBroadcaster.sendTransform(
        tf::StampedTransform(robotToSensor, ros::Time::now(), getName(), _sensors[i]->getFrameId()));
  }
}
