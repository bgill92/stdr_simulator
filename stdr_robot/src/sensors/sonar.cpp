...
@return void
**/
Sonar::Sonar(const nav_msgs::OccupancyGrid& map, const stdr_msgs::SonarSensorMsg& msg, const std::string& name,
             ros::NodeHandle& n)
  ...
  for (float sonarIter = angleMin; sonarIter < angleMax; sonarIter += angleStep)
  {
    ...
    while (distance <= _description.maxRange / _map.info.resolution)
    {
      xMap = _sensorTransform.getOrigin().x() / _map.info.resolution +
             cos(sonarIter + tf::getYaw(_sensorTransform.getRotation())) * distance;
      yMap = _sensorTransform.getOrigin().y() / _map.info.resolution +
             sin(sonarIter + tf::getYaw(_sensorTransform.getRotation())) * distance;
