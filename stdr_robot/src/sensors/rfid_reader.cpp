...
@return void
**/
RfidReader::RfidReader(const nav_msgs::OccupancyGrid& map, const stdr_msgs::RfidSensorMsg& msg, const std::string& name,
                       ros::NodeHandle& n)
  : Sensor(map, name, n, msg.pose, msg.frame_id, msg.frequency)
{
  _description = msg;

  _publisher = n.advertise<stdr_msgs::RfidSensorMeasurementMsg>(_namespace + "/" + msg.frame_id, 1);

  rfids_subscriber_ = n.subscribe("stdr_server/rfid_list", 1, &RfidReader::receiveRfids, this);
}
...
{
  if (rfid_tags_.rfid_tags.size() == 0)
    return;
  ...
  for (unsigned int i = 0; i < rfid_tags_.rfid_tags.size(); i++)
  {
    ...
    float dist =
        sqrt(pow(sensor_x - rfid_tags_.rfid_tags[i].pose.x, 2) + pow(sensor_y - rfid_tags_.rfid_tags[i].pose.y, 2));
    if (dist > max_range)
    {
      continue;
    }
    ...
    float ang = atan2(rfid_tags_.rfid_tags[i].pose.y - sensor_y, rfid_tags_.rfid_tags[i].pose.x - sensor_x);
