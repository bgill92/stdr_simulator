...
@return void
**/
CO2Sensor::CO2Sensor(const nav_msgs::OccupancyGrid& map, const stdr_msgs::CO2SensorMsg& msg, const std::string& name,
                     ros::NodeHandle& n)
  : Sensor(map, name, n, msg.pose, msg.frame_id, msg.frequency)
{
  _description = msg;

  _publisher = n.advertise<stdr_msgs::CO2SensorMeasurementMsg>(_namespace + "/" + msg.frame_id, 1);

  co2_sources_subscriber_ = n.subscribe("stdr_server/co2_sources_list", 1, &CO2Sensor::receiveCO2Sources, this);
}
...
{
  if (co2_sources_.co2_sources.size() == 0)
    return;
  ...
  for (unsigned int i = 0; i < co2_sources_.co2_sources.size(); i++)
  {
    ...
    if (dist > 0.5)
    {
      measuredSourcesMsg.co2_ppm += co2_sources_.co2_sources[i].ppm * pow(0.5, 2) / pow(dist, 2);
    }
    else
    {
