...
@return void
**/
Laser::Laser(const nav_msgs::OccupancyGrid& map, const stdr_msgs::LaserSensorMsg& msg, const std::string& name,
             ros::NodeHandle& n)
...
void Laser::updateSensorCallback()
{
  ...
  _laserScan.angle_increment = (_description.maxAngle - _description.minAngle) / divisions;
  ...
  {
    angle = tf::getYaw(_sensorTransform.getRotation()) + _description.minAngle +
            laserScanIter * (_description.maxAngle - _description.minAngle) / divisions;
    ...
    while (distance <= _description.maxRange / _map.info.resolution)
    {
      ...
      if (yMap * _map.info.width + xMap > _map.info.height * _map.info.width || yMap * _map.info.width + xMap < 0)
      {
