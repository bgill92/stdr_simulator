...
**/
ThermalSensor::ThermalSensor(const nav_msgs::OccupancyGrid& map, const stdr_msgs::ThermalSensorMsg& msg,
                             const std::string& name, ros::NodeHandle& n)
  : Sensor(map, name, n, msg.pose, msg.frame_id, msg.frequency)
{
  _description = msg;

  _publisher = n.advertise<stdr_msgs::ThermalSensorMeasurementMsg>(_namespace + "/" + msg.frame_id, 1);

  thermal_sources_subscriber_ =
      n.subscribe("stdr_server/thermal_sources_list", 1, &ThermalSensor::receiveThermalSources, this);
}
...
{
  if (thermal_sources_.thermal_sources.size() == 0)
    return;
  ...
  for (unsigned int i = 0; i < thermal_sources_.thermal_sources.size(); i++)
  {
    ...
    float dist = sqrt(pow(sensor_x - thermal_sources_.thermal_sources[i].pose.x, 2) +
    ...
    float ang = atan2(thermal_sources_.thermal_sources[i].pose.y - sensor_y,
    ...
    if (thermal_sources_.thermal_sources[i].degrees > measuredSourcesMsg.thermal_source_degrees[0])
    {
      measuredSourcesMsg.thermal_source_degrees[0] = thermal_sources_.thermal_sources[i].degrees;
    }
  }
...
**/
void ThermalSensor::receiveThermalSources(const stdr_msgs::ThermalSourceVector& msg)
{
