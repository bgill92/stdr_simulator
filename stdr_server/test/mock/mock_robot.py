...
class MockRobot:
    def __init__(self):
        self.load_nodelet = rospy.Service(
            "robot_manager/load_nodelet", NodeletLoad, self.load_nodelet_cb
        )

        self.unload_nodelet = rospy.Service(
            "robot_manager/unload_nodelet", NodeletUnload, self.unload_nodelet_cb
        )

        self.register_robot_client = SimpleActionClient(
            "stdr_server/register_robot", RegisterRobotAction
        )
    ...
    def register_robot(self, name):
        ...
        self.move_service = rospy.Service(name + "/replace", MoveRobot, self.move_robot)
