...
[[nodiscard]] tl::expected<nav_msgs::msg::OccupancyGrid, std::string> load_map(const std::string& yaml_path)
{
  ...
  grid.info.width = static_cast<uint32_t>(image.width);
  grid.info.height = static_cast<uint32_t>(image.height);
  grid.info.resolution = static_cast<float>(meta.resolution);
  grid.info.origin.position.x = meta.origin.x;
  grid.info.origin.position.y = meta.origin.y;
  grid.info.origin.position.z = 0.0;

  // Encode the yaw angle as a Z-axis quaternion.
  const double half_theta = meta.origin.theta / 2.0;
  grid.info.origin.orientation.x = 0.0;
  grid.info.origin.orientation.y = 0.0;
  grid.info.origin.orientation.z = std::sin(half_theta);
  grid.info.origin.orientation.w = std::cos(half_theta);
