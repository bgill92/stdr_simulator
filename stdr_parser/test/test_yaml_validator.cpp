...
TEST(YamlValidator, ValidRobotPasses) {
  const YAML::Node doc =
      YAML::LoadFile(std::string{kFixtureDir} + "/valid_robot.yaml");
  const ValidationResult result =
      validate_yaml(doc["robot"], "robot", std::string{kSpecsDir});
...
TEST(YamlValidator, UnknownKeyDetected) {
  const YAML::Node doc =
      YAML::LoadFile(std::string{kFixtureDir} + "/invalid_robot_typo.yaml");
  const ValidationResult result =
      validate_yaml(doc["robot"], "robot", std::string{kSpecsDir});
...
TEST(YamlValidator, MissingRequiredKeyDetected) {
  const YAML::Node doc = YAML::LoadFile(std::string{kFixtureDir} +
                                        "/invalid_robot_missing_required.yaml");
  const ValidationResult result =
      validate_yaml(doc["robot"], "robot", std::string{kSpecsDir});
...
TEST(YamlValidator, MultipleLasersAllowed) {
  ...
  const YAML::Node doc =
      YAML::LoadFile(std::string{kFixtureDir} + "/robot_multiple_lasers.yaml");
  const ValidationResult result =
      validate_yaml(doc["robot"], "robot", std::string{kSpecsDir});
...
TEST(YamlValidator, EmptyNodeReturnsValid) {
  ...
  const ValidationResult result =
      validate_yaml(empty_node, "unknown_type", std::string{kSpecsDir});
