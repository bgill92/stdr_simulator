...
TEST(Parser, LoadRobotMsgReturnsErrorForMissingFile) {
  const tl::expected<stdr_msgs::msg::RobotMsg, std::string> result =
      load_robot_msg("/nonexistent/path/robot.yaml",
                     "/nonexistent/base",
                     FIXTURE_DIR "/specifications");
...
TEST(Parser, LoadRobotMsgReturnsValidationError) {
  ...
  const tl::expected<stdr_msgs::msg::RobotMsg, std::string> result =
      load_robot_msg(FIXTURE_DIR "/invalid_robot_typo.yaml",
                     FIXTURE_DIR,
                     FIXTURE_DIR "/specifications");
  EXPECT_FALSE(result.has_value());
...
TEST(Parser, LoadRobotMsgFromValidFile) {
  const tl::expected<stdr_msgs::msg::RobotMsg, std::string> result =
      load_robot_msg(FIXTURE_DIR "/simple_robot.yaml",
                     FIXTURE_DIR,
                     FIXTURE_DIR "/specifications");
...
TEST(Parser, LoadMapReturnsErrorForMissingFile) {
  const tl::expected<stdr_simulation::MapMetadata, std::string> result =
      load_map("/nonexistent/path/map.yaml");
  EXPECT_FALSE(result.has_value());
}

TEST(Parser, LoadMapReturnsMetadata) {
  const tl::expected<stdr_simulation::MapMetadata, std::string> result =
      load_map(FIXTURE_DIR "/test_map.yaml");
  ASSERT_TRUE(result.has_value()) << result.error();
