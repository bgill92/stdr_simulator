...
{
  const YAML::Node root = YAML::LoadFile(specs_dir + "/stdr_specifications.yaml");
...
[[nodiscard]] std::set<std::string> load_multiple_allowed(const std::string& specs_dir)
{
  const YAML::Node root = YAML::LoadFile(specs_dir + "/stdr_multiple_allowed.yaml");
...
void validate_node(const YAML::Node& node, const std::string& context_type, const std::string& path, const Specs& specs,
                   const std::set<std::string>& multiple_allowed, ValidationResult& result);

/** @brief Validate a map node: check allowed keys, required keys, and recurse. */
void validate_map(const YAML::Node& node, const std::string& context_type, const std::string& path, const Specs& specs,
                  const std::set<std::string>& multiple_allowed, ValidationResult& result)
{
  ...
  if (it != specs.end())
  {
    ...
    for (YAML::const_iterator it = node.begin(); it != node.end(); ++it)
    {
      ...
      {
        result.valid = false;
        result.errors.push_back("Unknown key '" + key + "' at " + path + " (allowed: " + join_set(spec.allowed) + ")");
      }
      // Recurse using the key as the new context type.
      validate_node(val_node, key, path + "." + key, specs, multiple_allowed, result);
    }
    ...
    if (!node["filename"])
    {
      for (const std::string& req : spec.required)
      {
        ...
        {
          result.valid = false;
          result.errors.push_back("Missing required key '" + req + "' at " + path);
        }
      }
    }
  }
...
void validate_sequence(const YAML::Node& node, const std::string& context_type, const std::string& path,
                       const Specs& specs, const std::set<std::string>& multiple_allowed, ValidationResult& result)
{
  for (std::size_t i = 0; i < node.size(); ++i)
  {
    validate_node(node[i], context_type, path + "[" + std::to_string(i) + "]", specs, multiple_allowed, result);
  }
}
                   ...
                   const std::set<std::string>& multiple_allowed, ValidationResult& result)
{
  ...
  {
    validate_sequence(node, context_type, path, specs, multiple_allowed, result);
  }
...
ValidationResult validate_yaml(const YAML::Node& yaml_node, const std::string& root_type, const std::string& specs_dir)
{
  ...
  const std::set<std::string> multiple_allowed = load_multiple_allowed(specs_dir);

  validate_node(yaml_node, root_type, root_type, specs, multiple_allowed, result);
  return result;
}
