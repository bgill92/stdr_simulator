...
tl::expected<stdr_msgs::msg::RobotMsg, std::string> load_robot_msg(
    ...
    const std::string& specs_dir)
{
  ...
  if (!validation.valid) {
    const std::string joined = std::accumulate(
        std::next(validation.errors.begin()),
...
tl::expected<stdr_simulation::MapMetadata, std::string> load_map(
    const std::string& yaml_path)
{
